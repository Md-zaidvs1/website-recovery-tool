...
export class ExtractionEngine {
  ...
  static async getWaybackSnapshots(domain: string, signal?: AbortSignal, logs?: string[]): Promise<{ timestamp: string; original: string; date: string; }[]> {
    ...
    try {
      ...
      const response = await axios.get(cdxUrl, { timeout: 8000, signal });
    ...
    if (snapshots.length === 0) {
      try {
        ...
        const response = await axios.get(waybackApiUrl, { timeout: 8000, signal });
   ...
   */
  private static cleanAndValidatePhone(raw: string, logs?: string[]): string | null {
    const clean = raw.trim();
    if (!clean) return null;

    // Check if it's a decimal number or contains a float structure
    // e.g. 46.6666666666, 1.23, etc.
    if (/\d+\.\d+/.test(clean)) {
      // Allow dot-separated numbers like 123.456.7890 but reject real decimals/floats
      ...
      if (dots > 1) {
        // Dot-separated phone e.g. "123.456.7890". Let's check if parts are just digits
    ...
    // Reject common JavaScript/CSS dynamic values or wix/sentry identifiers
    const lowercase = clean.toLowerCase();
    if (
      lowercase.includes('px') || 
      lowercase.includes('em') || 
      lowercase.includes('%') || 
      lowercase.includes('vw') || 
      lowercase.includes('vh') || 
      ...
      this.logRejectedPhone(raw, 'HTML/CSS property or framework identifier detected', logs);
      return null;
    }
    ...
    // Reject numbers shorter than 10 digits
    if (digits.length < 10) {
      this.logRejectedPhone(raw, 'Too few digits', logs);
      return null;
    }

    // Reject numbers longer than 15 digits
    if (digits.length > 15) {
      this.logRejectedPhone(raw, 'Too many digits', logs);
      return null;
    }

    // Reject Unix timestamps in seconds (10 digits starting with 13, 14, 15, 16, 17, 18, 19)
    if (digits.length === 10 && /^(13|14|15|16|17|18|19)/.test(digits)) {
      this.logRejectedPhone(raw, 'Unix timestamp detected', logs);
      return null;
    }

    // Reject Unix timestamps in milliseconds (13 digits starting with 13, 14, 15, 16, 17, 18, 19)
    ...
    // Reject other typical timestamps/IDs
    ...
    // Mobile format check: 10 digits starting with 6, 7, 8, or 9
    const isIndianMobile10 = digits.length === 10 && /^[6789]/.test(digits);
    if (isIndianMobile10) {
      return `+91 ${digits}`;
    }

    // Mobile with 91 prefix: 12 digits starting with 91, then 6, 7, 8, or 9
    const isIndianMobile12 = digits.length === 12 && /^91[6789]/.test(digits);
    if (isIndianMobile12) {
      return `+91 ${digits.substring(2)}`;
    }

    // Mobile with leading 0: 11 digits starting with 0, then 6, 7, 8, or 9
    const isIndianMobile11 = digits.length === 11 && /^0[6789]/.test(digits);
    if (isIndianMobile11) {
      return `+91 ${digits.substring(1)}`;
    }

    // Landline format starting with 0: e.g. 01123456789
    ...
    // If it already has +91 with spaces/hyphens
    ...
    // General fallback normalization for international or other numbers
    if (clean.startsWith('+')) {
      return `+${digits}`;
    }
  ...
  private static extractPhones($: cheerio.CheerioAPI, html: string, isContactPage = false, logs?: string[]): string[] {
    const phonesMap = new Map<string, 'HIGH' | 'LOW'>();

    const addPhone = (raw: string, confidence: 'HIGH' | 'LOW') => {
      const normalized = this.cleanAndValidatePhone(raw, logs);
      if (normalized) {
    ...
    $('a').each((_, el) => {
      ...
      if (href.includes('wa.me') || href.includes('api.whatsapp.com') || href.includes('whatsapp.com/send') || href.startsWith('whatsapp://')) {
        ...
        if (rawPhone) {
          addPhone(rawPhone, 'HIGH');
        } else {
          // Fallback parsing
          const match = cleanHref.match(/(?:\?phone=|send\?phone=|\/)([\d+]+)/);
          if (match && match[1]) {
            addPhone(match[1], 'HIGH');
          }
        }
      }
    });

    // We'll use a regex that matches standard numbers but can be filtered by the validator
    const phoneRegex = /(?:\+?\d{1,4}[-.\s]?)?\(?\d{2,4}\)?[-.\s]?\d{3,4}[-.\s]?\d{4}/g;

    // Helper to find numbers in any block of text with boundaries checking
    const scanBlock = (text: string, baseConfidence: 'HIGH' | 'LOW') => {
      let match;
      ...
      while ((match = phoneRegex.exec(text)) !== null) {
        ...
        // Check boundary characters to reject part of a decimal/ID
        ...
        addPhone(matchedText, baseConfidence);
      }
    };

    // 3. Scan high-confidence blocks (Address, Footer, Contacts)
    ...
    highConfidenceSelectors.forEach(selector => {
      $(selector).each((_, el) => {
        ...
        scanBlock($el.text(), 'HIGH');
      });
    });

    // 4. Scan all other text nodes (LOW confidence, unless isContactPage is true)
    const $clean = cheerio.load(html);
    $clean('script, style, noscript, iframe, head, meta, link, svg, path, canvas').remove();
    const pageText = $clean('body').text();
    
    scanBlock(pageText, isContactPage ? 'HIGH' : 'LOW');

    // Sort: HIGH confidence first, then LOW confidence.
