...
export class BulkService {
  ...
  static startWorkerForBatch(batchId: string): void {
    ...
    const concurrencyLimit = existingJob?.options?.concurrency || 8;
  ...
  private static async runWorkerLoop(batchId: string, workerIndex: number): Promise<void> {
    ...
    while (true) {
      ...
      Domains.findByIdAndUpdate(domainRecord.id, { status: 'processing', startedAt: new Date().toISOString() } as any);
      ...
      const userTimeout = options.timeout || 30;
      // If Gemini grounding is enabled, extend the watchdog timeout to allow for Gemini rate limit spacing and API responses
      const timeoutSec = options.geminiGrounding !== false ? Math.max(userTimeout, 90) : userTimeout;
      ...
      if (isTimeout) {
        // Watchdog failure: retry if retryCount < 2, otherwise mark as failed
        if (domainRecord.retryCount < 2) {
          ...
          this.addJobLog(batchId, `Retrying ${domainName} due to timeout (attempt ${domainRecord.retryCount + 1}/2)...`);
          ...
          this.addJobLog(batchId, `Failed processing ${domainName} due to persistent timeout: ${errMsg}`);
  ...
  static syncJobProgress(batchId: string): void {
    ...
    if (index >= 0) {
      jobs[index] = {
        ...
        workers: this.activeBatchIds.has(batchId) ? (jobs[index]?.options?.concurrency || 8) : 0,
      };
      this.writeBulkJobs(jobs);
    }
  }
  ...
  static getBatchProgress(batchId: string) {
    ...
    const processingDomains = batchDomains.filter((d) => d.status === 'processing');
    const currentTargets = processingDomains.slice(0, 5).map((d: any) => ({
      domain: d.domain,
      source: 'cheerio',
      startedAt: d.startedAt ? new Date(d.startedAt) : new Date(d.updatedAt || d.createdAt || Date.now())
    }));
